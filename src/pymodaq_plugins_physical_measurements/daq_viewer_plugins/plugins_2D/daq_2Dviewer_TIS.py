...
class DAQ_2DViewer_TIS(DAQ_Viewer_base):
    ...
    *y_axis*         1D numpy array
    ...
    ic.init_library()
    cameras = [cam for cam in ic.get_unique_device_names()]
    ...
    params = comon_parameters + [
        ...
        {"title": "Gray scale:", "name": "gray_scale", "type": "bool", "value": False},
        {
            ...
            "children": [
                ...
                {
                    "title": "Black light compensation:",
                    "name": "blacklightcompensation",
                    "type": "int",
                },
    ...
    def __init__(
        self, parent=None, params_state=None
    ...
    def _data_ready(self, handle_ptr, p_data, frame_num, data):
        ...
        if self.settings.child("gray_scale").value():
            self.data_grabed_signal.emit(
                [
                    DataFromPlugins(name="TIS ", data=[data[:, :, 0]], dim="Data2D"),
                ]
            )
        else:
            self.data_grabed_signal.emit(
        ...
        *param*          instance of pyqtgraph Parameter   the parameter to activate
        ...
        try:
            if param.parent().name() == "cam_settings":
                ...
                self.controller.set_video_format(param.value())
    ...
    def ini_detector(self, controller=None):
        ...
        """
        self.status.update(
            edict(initialized=False, info="", x_axis=None, y_axis=None, controller=None)
        )
        try:
            ...
            if self.settings.child(("controller_status")).value() == "Slave":
                ...
                self.controller = self.ic.get_device(
                    self.settings.child(("cam_name")).value()
                )
            ...
            formats = [
                form for form in self.controller.list_video_formats()
            ]  # if 'RGB'.encode() in form]
            self.settings.child(("video_formats")).setOpts(limits=formats)
            self.settings.child(("video_formats")).setValue(formats[0])
            self.controller.set_video_format(
                formats[0]
            )  # use first available video format
